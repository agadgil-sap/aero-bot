...
if pgrep -f "aero-bot-teacher" >/dev/null 2>&1; then
...
# All three stay unloaded until the operator bootstraps them. The baked PATH
# carries ~/.local/bin and Homebrew because launchd's bare PATH has neither,
# and the teacher CLIs (claude, codex, uv) live there.
generate_plist() {
    local label="$1"
    local stream="$2"
