...
The installer writes `com.aero-bot.teacher-{tactical,daily,news,hindsight,upgrade,risk-manager}.plist` into `~/Library/LaunchAgents` and never loads them; arming each job is the operator's own `launchctl bootstrap` line (printed by the installer, documented in the teacher guide).
