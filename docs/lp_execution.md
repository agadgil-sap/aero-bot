...
A unit of liquidity is valued over the range to split the budget into both sides; desired amounts floor to raw units. Mint minima are geometric rather than a flat percentage: the planner re-solves executable liquidity at both ends of a +/-0.35-tick execution-price envelope and uses the smallest token amounts across those endpoints, with a one-raw-unit rounding cushion. The envelope is calibrated from the measured AAPLc (~0.059 tick) and MSTRc (~0.312 tick) post-estimate PSC failures; if either endpoint would reduce executable liquidity below 95 percent of the anchor plan, the mint refuses instead of weakening minima further. The 1-percent slippage parameter remains the swap-output tolerance and the exit/decrease tolerance, not the concentrated mint's composition tolerance.
...
6. The receipt is awaited under one bounded 600-second wait, polling every configured endpoint round-robin (the primary plus `base.publicnode.com`, `1rpc.io/base`, and `base.drpc.org`) every three seconds. Exhaustion reports the step as `unconfirmed` with a warning diagnostic and halts the sequence - never relabeled a failure, because a landed broadcast was mislabeled once before. An included-but-reverted delivery audits `lp_execute_failed` and exits one; before recording that failure the executor replays the exact inner target/calldata read-only from the Safe at both the receipt block and its parent. This bypasses Safe's outer `GS013` wrapper and records decoded Solidity `Error(string)` / `Panic(uint256)` evidence, or the custom-error selector when no ABI is available; replay disagreement is explicitly labeled state-sensitive. A confirmed delivery audits `lp_execute_confirmed` and proceeds to the next nonce.
...
The mint step itself never broadcast: its fresh estimate refused honestly four times as `estimate_reverted` with the inner `PSC` revert. That historical campaign traced the failure to the old flat 0.1-percent-of-amount minima convention on a 20-tick range. The live planner has since replaced flat mint minima with the bounded geometric execution-price envelope described above; later live evidence on 2026-09-19 exposed `PSC` directly by replaying the failed NFPM inner call and expanded the calibrated envelope to 0.35 tick while retaining the 95-percent utilization floor.
